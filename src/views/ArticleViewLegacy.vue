  ...
  return ARTICLE_PREFIXES.map(prefix => ({
    prefix,
        articles: articles.value.filter(article => {
            ...
            return article.name.startsWith(prefix) && !article.error
            // console.log(article)
            // return {
    ...
    >
      <template v-if="articles.length > 0">
        ...
        >
                    ...
                    <div v-if="group.prefix == '00-'" class="group-title">RANDOM</div>
                    <div v-if="group.prefix == '01-'" class="group-title">VUEJS</div>
                    <div v-if="group.prefix == '02-'" class="group-title">LOWLEVEL</div>
...
.group-title { text-align: left; font-size: 2rem; margin-bottom: 20px;
    padding-left: 5px; }
</style>
