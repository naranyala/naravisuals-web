                    ...
                    <ul>
                        {items.iter().map(|item| {
                            ...
                            if is_task {
                                ...
                                view! { <li>{Self::render_children(&item.children)}</li> }.into_view()
                            }
                        }).collect::<Vec<_>>()}
                    </ul> 
                }.into_view()
                ...
                view! {
                    <div class="table-wrapper">
                        <table>
                            <thead>
                                <tr>
                                    {headers.iter().map(|h| view! { <th>{Self::render_inline(std::slice::from_ref(h))}</th> }).collect::<Vec<_>>()}
                                </tr>
                            </thead>
                            <tbody>
                                {rows.iter().map(|row| view! {
                                    <tr>
                                        {row.iter().map(|cell| view! { <td>{Self::render_inline(std::slice::from_ref(cell))}</td> }).collect::<Vec<_>>()}
                                    </tr>
                                }).collect::<Vec<_>>()}
                            </tbody>
                        </table>
