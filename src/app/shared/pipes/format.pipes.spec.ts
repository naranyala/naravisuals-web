...
import { FilterByPipe, NumberFormatPipe } from './format.pipes';
import { TitleCasePipe, TruncatePipe } from './string.pipes';
...
describe('TruncatePipe', () => {
  ...
  test('should truncate text to specified length', () => {
    ...
    expect(pipe.transform('This is a long text', 13)).toBe('This is a ...');
  });

  test('should use custom ellipsis', () => {
    expect(pipe.transform('Hello World', 8, ' [more]')).toBe('Hello [more]');
  });
  ...
  test('should use default limit behavior', () => {
    const longText = 'a'.repeat(60);
    expect(pipe.transform(longText, 53)).toBe(`${'a'.repeat(50)}...`);
  });
});
