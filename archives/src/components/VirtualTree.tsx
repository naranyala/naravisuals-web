...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const flattenNodes = (nodes: TreeNode[], level: number = 0): any[] => {
      ...
      nodes.forEach((node) => {
        result.push({
          ...
          visible:
            level === 0 ||
            expandedKeys.value.has(node.id.split('-').slice(0, -1).join('-')),
        });
    ...
    const visibleNodes = computed(() =>
      flatNodes.value.filter((node) => node.visible),
    );
          ...
          {visibleNodes.value.map((node) => (
            ...
            >
              <div class={styles.nodeContent}>
                ...
                {node.icon && <div class={styles.icon}>{node.icon}</div>}
