        ...
        <div class={styles.row}>
          {props.sections.map((sec) => (
            <section
              ...
              class={clsx(
                styles.sectionCard,
                props.compact && styles.compactSectionCard,
              )}
            >
              <header class={styles.sectionHeader}>
                <div class={styles.textCenter}>
                  ...
                  {sec.subtitle ? (
                    <div class={styles.subtitle}>{sec.subtitle}</div>
                  ) : null}
                </div>
              </header>
              ...
              <ul class={styles.itemList}>
                {sec.items.map((item, idx) => (
                  <li
                    ...
                    class={clsx(
                      styles.item,
                      props.compact && styles.compactItem,
                    )}
                    style={{ textAlign: 'center', width: '100%' }}
                    onClick={() =>
                      actionClick({ context: sec.title, selected: item })
                    }
                  >
