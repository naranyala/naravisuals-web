...
function updateVitePressConfig(docsPath) {
  const configPath = join(docsPath, '.vitepress', 'config.ts')
  const sidebar = generateSidebar(docsPath)
  
  const configContent = `import { defineConfig } from 'vitepress'

// Auto-generated sidebar configuration
// To customize order, add 'order: N' to the frontmatter of your markdown files
export default defineConfig({
  title: "Tauri Backend Guide",
  description: "Comprehensive guide to Rust Tauri backend development",
  themeConfig: {
  ...
  try {
    ...
    console.log('✅ VitePress config updated successfully!')
    console.log(`📁 Found ${sidebar[0]?.items?.length || 0} documentation items`)
  } catch (error) {
...
updateVitePressConfig(docsPath)

export { generateSidebar, updateVitePressConfig }