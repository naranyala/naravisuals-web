...
mod tests {
    ...
    fn test_tokenize_inline_elements() {
        ...
        let tokens = tokenize(input);
        assert!(tokens.contains(&Token::BoldMarker));
        ...
        assert!(tokens_imbalanced.contains(&Token::BoldMarker));
    }

    #[test]
    fn test_tokenize_nested_inline() {
    ...
    #[test]
    fn test_tokenize_empty_inline_code() {
        let input = "Empty code ``";
        let tokens = tokenize(input);
        assert!(tokens.contains(&Token::InlineCode("".to_string())));
    }
    ...
    fn test_tokenize_lists() {
        let input = "- item 1\n1. item 2";
        let tokens = tokenize(input);
        assert_eq!(tokens[0], Token::UnorderedListMarker);
        assert_eq!(tokens[2], Token::BlankLine);
        assert_eq!(tokens[3], Token::OrderedListMarker);

        // Test two unordered lists
        let input_unordered = "- item 1\n- item 2";
        let tokens_unordered = tokenize(input_unordered);
        assert_eq!(tokens_unordered[0], Token::UnorderedListMarker);
        assert_eq!(tokens_unordered[2], Token::BlankLine);
        assert_eq!(tokens_unordered[3], Token::UnorderedListMarker);

        // Edge case: Malformed ordered list
        let tokens_malformed = tokenize("not_a_number. item");
        assert!(!matches!(tokens_malformed[0], Token::OrderedListMarker));
    }
    ...
    fn test_tokenize_extensions() {
        ...
        if let Token::ExtensionStart { attributes, .. } = &tokens_attrs[0] {
            ...
            panic!("Expected ExtensionStart with attributes");
        }
    }
