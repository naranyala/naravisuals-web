...
pub fn extract_all_text(node: &Node) -> String {
    match node {
        ...
        Node::UnorderedList(items) => items.iter().map(|i| extract_all_text(&Node::ListItem(i.children.clone()))).collect(),
        Node::OrderedList(items) => items.iter().map(|i| extract_all_text(&Node::ListItem(i.children.clone()))).collect(),
        ...
        Node::Table { headers, rows } => {
            let mut text = headers.iter().map(extract_all_text).collect::<Vec<_>>().join(" ");
            text.push(' ');
            text.push_str(&rows.iter().map(|row| row.iter().map(extract_all_text).collect::<Vec<_>>().join(" ")).collect::<Vec<_>>().join(" "));
            text
        }
        Node::HorizontalRule => String::new(),
        Node::Extension { children, content, .. } => {
            let mut res = content.clone();
            res.push(' ');
            res.push_str(&children.iter().map(extract_all_text).collect::<Vec<_>>().join(" "));
            res
        }
