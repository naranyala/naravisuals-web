...
*   **Before v10**: `mermaid.render('id', source)` returned an SVG string immediately.
*   **v10 & v11**: `await mermaid.render('id', source)` returns a Promise resolving to an object `{ svg, bindFunctions }`.
*   **Compiler Impact**: Your React components must use `useEffect` and `async/await` to handle the rendering lifecycle, and must manage "loading" states to prevent race conditions.

### 2. ESM & Module Loading
Starting with v10, Mermaid transitioned fully to **ES Modules**.
*   **Impact**: Dynamic `import("mermaid")` became the preferred way to load the library in SPAs, reducing initial bundle size but requiring careful handling of global state.
...
*   **Subgraphs**: Much more robust support for nested subgraphs and cross-subgraph links.
*   **Styling**: Better support for CSS classes and inline styles.
*   **Markers**: Supports advanced arrow heads and bidirectional links.
*   **Direction**: Handles `TB` (Top to Bottom), `BT`, `LR`, `RL` with higher consistency.

### The "Graph" (Legacy)
Kept for backwards compatibility but missing several modern features.
*   **Limitations**: Frequent layout issues when mixing different node shapes with subgraphs.
*   **Validation**: Our build system now issues a **Warning** when `graph` is detected, encouraging authors to switch to `flowchart`.
...
*   **Solution**: Always prefix IDs with a string (e.g., `mermaid-chart-`) and include a unique hash or index.

### 2. Character Encoding
Characters like `>` in arrows or `"` in labels can be corrupted during the transformation from `.md` → `.html` → `.ts`.
*   **The Trap**: Markdown parsers often turn `-->` into `--&gt;`. If passed directly to Mermaid, it will fail to recognize the arrow.
*   **Mitigation**: Use a `data-source` attribute to store the "raw" unescaped notation in the generated HTML.
...
*   **The Conflict**: This often happens before React is ready, leading to "Unknown Diagram" errors when React tries to re-render the already-modified DOM.
*   **Fix**: Always initialize with `{ startOnLoad: false }` in your `index.html`.
